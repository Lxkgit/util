...
class PointPicker(QDialog):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Escape:
            self.reject()
        elif event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter) and self.point:
            self._confirm()
        else:
    ...
    def _update_position_text(self, pos, selected=False):
        if selected:
            self.info.setText(f"已选择：X={pos.x()}  Y={pos.y()} · 可重新点击选择")
        else:
...
class MouseActionDialog(QDialog):
    def __init__(self, action: str, parent=None):
        ...
        self.point: tuple[int, int] | None = None
        self.point_preview: PointPreviewOverlay | None = None
        ...
        form = QFormLayout()
        self.delay = QDoubleSpinBox(); self.delay.setRange(0, 999999); self.delay.setDecimals(3); self.delay.setSingleStep(0.01); self.delay.setSuffix(" 秒")
        ...
        self.position_label.setStyleSheet("font-weight:700;color:#303133;"); root.addWidget(self.position_label)
        actions = QHBoxLayout()
        pick = QPushButton("🎯 选择位置"); pick.clicked.connect(self.pick_position); actions.addWidget(pick)
        self.preview_position_button = QPushButton("👁 预览位置"); self.preview_position_button.setEnabled(False); self.preview_position_button.clicked.connect(self.preview_position); actions.addWidget(self.preview_position_button)
    ...
    def preview_position(self):
        ...
        if self.point_preview:
            self.point_preview.close(); self.point_preview.deleteLater()
...
class ScreenPreviewOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._cursor_pos = None; self._title = "安全预览"; self._detail = ""; self._warning = "不会执行任何真实键盘或鼠标操作"
        self.setGeometry(self._geometry)
        self.setWindowFlags(Qt.WindowType.FramelessWindowHint | Qt.WindowType.WindowStaysOnTopHint | Qt.WindowType.Tool | Qt.WindowType.WindowDoesNotAcceptFocus)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground); self.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents); self.setAttribute(Qt.WidgetAttribute.WA_ShowWithoutActivating)

    def set_action(self, title, detail, cursor_pos=None):
        self._title = title; self._detail = detail
        if cursor_pos is not None: self._cursor_pos = cursor_pos
        self.update()

    def paintEvent(self, event):
        painter = QPainter(self); painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        panel = self.rect().adjusted(24, 24, -24, -24); panel.setHeight(128)
        painter.setPen(Qt.PenStyle.NoPen); painter.setBrush(Qt.GlobalColor.black); painter.setOpacity(0.88); painter.drawRoundedRect(panel, 12, 12); painter.setOpacity(1)
        painter.setPen(Qt.GlobalColor.white); font = painter.font(); font.setPointSize(18); font.setBold(True); painter.setFont(font); painter.drawText(panel.adjusted(20, 16, -20, 0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop, self._title)
        font.setPointSize(14); font.setBold(False); painter.setFont(font); painter.drawText(panel.adjusted(20, 50, -20, -36), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop, self._detail)
        font.setPointSize(12); font.setBold(True); painter.setFont(font); painter.setPen(Qt.GlobalColor.yellow); painter.drawText(panel.adjusted(20, -24, -20, -12), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignBottom, self._warning)
        if self._cursor_pos:
            x, y = self._cursor_pos; px, py = x - self._geometry.left(), y - self._geometry.top()
            painter.setPen(QPen(Qt.GlobalColor.red, 3)); painter.drawLine(px - 18, py, px + 18, py); painter.drawLine(px, py - 18, px, py + 18)
            painter.setPen(QPen(Qt.GlobalColor.white, 2)); painter.drawEllipse(px - 8, py - 8, 16, 16)
        painter.end()
...
class MacroEditorDialog(QDialog):
    def __init__(self, events: list[MacroEvent], parent=None):
        ...
        self._preview_timer = QTimer(self); self._preview_timer.setSingleShot(True); self._preview_timer.timeout.connect(self._preview_timer_tick)
        self._preview_index = 0; self._preview_running = False; self._preview_overlay = None
        root = QVBoxLayout(self); root.setContentsMargins(22, 18, 22, 18); root.setSpacing(12)
        head = QHBoxLayout(); title_box = QVBoxLayout(); title = QLabel("宏编辑器"); title.setStyleSheet("font-size:24px;font-weight:700;"); subtitle = QLabel("键盘可以直接添加；鼠标操作通过全屏选点确定位置。"); subtitle.setStyleSheet("color:#7a8491;"); title_box.addWidget(title); title_box.addWidget(subtitle); head.addLayout(title_box, 1)
        self.preview_button = QPushButton("▶ 桌面预览"); self.preview_button.clicked.connect(self.toggle_preview); head.addWidget(self.preview_button); root.addLayout(head)
        toolbar = QHBoxLayout()
    ...
    def delete_selected(self):
        rows = self.list.selectedItems()
        if not rows: return
        self.events.pop(self.list.row(rows[0])); self._reload_list(min(self.list.row(rows[0]), len(self.events) - 1))
