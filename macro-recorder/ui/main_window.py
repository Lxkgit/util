...
from PySide6.QtWidgets import QApplication, QFileDialog, QFrame, QHBoxLayout, QLabel, QListWidget, QMainWindow, QMessageBox, QProgressBar, QPushButton, QSpinBox, QVBoxLayout, QWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.events: list[MacroEvent] = []; self.current_file=""; self.record_mode="all"; self._pending=False
        settings=QSettings("Lxkgit","MacroRecorder")
        self.record_hotkey=str(settings.value("record_hotkey","F8")); self.stop_hotkey=str(settings.value("stop_hotkey","F8")); self.shared_hotkey=settings.value("shared_hotkey",True,type=bool)
        self.recorder=MacroRecorder(self.event_signal.emit); self.player=MacroPlayer(self._progress,self._state); self.hotkeys=HotkeyService(self.action_signal.emit)
        self._build_ui(); self.event_signal.connect(self._on_event); self.action_signal.connect(self._on_action); self._install_hotkeys(); self._refresh()

    def _build_ui(self):
        root=QWidget(); root.setStyleSheet("""
            QWidget{font-size:14px;} QMainWindow{background:#f5f7fa;} QPushButton{min-height:38px;padding:0 16px;border:1px solid #dcdfe6;border-radius:8px;background:white;} QPushButton:hover{background:#f2f6fc;} QPushButton:disabled{color:#a8abb2;background:#f5f7fa;} QFrame#panel,QFrame#card{background:white;border:1px solid #e4e7ed;border-radius:12px;} QFrame#modeCard{background:white;border:1px solid #e4e7ed;border-radius:12px;} QFrame#modeCard[selected="true"]{border:2px solid #409eff;background:#f4f9ff;} QLabel#modeTitle{font-size:16px;font-weight:700;} QLabel#modeIcon{font-size:28px;} QListWidget#eventList{background:white;border:1px solid #e4e7ed;border-radius:10px;padding:6px;color:#303133;} QListWidget#eventList::item{padding:8px 10px;color:#303133;} QListWidget#eventList::item:selected{color:#303133;background:#eaf2ff;} QSpinBox{min-height:34px;border:1px solid #dcdfe6;border-radius:6px;padding:0 8px;} QProgressBar{height:8px;border:0;border-radius:4px;background:#e9edf2;}
        """)
        ...
        for mode,icon,name,desc in self.MODES:
            card=QFrame(); card.setObjectName("modeCard"); card.setProperty("selected",mode==self.record_mode); card.mousePressEvent=lambda e,m=mode:self.select_mode(m)
            b=QVBoxLayout(card); b.setContentsMargins(18,14,18,14); il=QLabel(icon); il.setObjectName("modeIcon"); nl=QLabel(name); nl.setObjectName("modeTitle"); dl=QLabel(desc); dl.setStyleSheet("color:#7a8491;"); b.addWidget(il); b.addWidget(nl); b.addWidget(dl); modes.addWidget(card); self.mode_cards[mode]=card
        layout.addLayout(modes)
        panel=QFrame(); panel.setObjectName("panel"); controls=QHBoxLayout(panel); controls.setContentsMargins(14,12,14,12)
        self.record_btn=QPushButton("●  开始录制"); self.play_btn=QPushButton("▶  播放"); self.pause_btn=QPushButton("Ⅱ  暂停"); self.stop_btn=QPushButton("■  停止"); self.clear_btn=QPushButton("清空")
        self.record_btn.clicked.connect(self.toggle_record); self.play_btn.clicked.connect(self.play); self.pause_btn.clicked.connect(self.player.toggle_pause); self.stop_btn.clicked.connect(self.stop_all); self.clear_btn.clicked.connect(self.clear_events)
    ...
    def select_mode(self, mode):
        ...
        labels={"all":"键盘 + 鼠标","keyboard":"仅键盘","mouse":"仅鼠标"}; self.status.setText(f"已选择：{labels[mode]}")

    def toggle_record(self):
        self.stop_recording() if self.recorder.recording else self.start_recording()
    ...
    def _state(self,state): self.status.setText(state); QTimer.singleShot(0,self._refresh)
    ...
    def open_settings(self):
        self.stop_recording() if self.recorder.recording else None; self._remove_hotkeys(); dialog=SettingsDialog(self.record_hotkey,self.stop_hotkey,self.shared_hotkey,self)
        if dialog.exec():
    ...
    def load(self):
        ...
        if not path:return
        try:
            self.events=load_macro(path); self.list.clear(); [self.list.addItem(self.format_event(e)) for e in self.events]; self.current_file=path; self.file_label.setText(os.path.basename(path)); self.progress.setValue(0); self.status.setText(f"加载成功，共 {len(self.events)} 个操作"); self._refresh()
        except Exception as exc: QMessageBox.critical(self,"加载失败",str(exc))

    def clear_events(self):
        self.events.clear(); self.list.clear(); self.progress.setValue(0); self.current_file=""; self.file_label.setText("尚未保存"); self.status.setText("已清空"); self._refresh()
    ...
    def _refresh(self):
        recording=self.recorder.recording; playing=self.player.running
